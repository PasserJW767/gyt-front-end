<template>
      ...
      <div  style="height:39px   ;background: #775637;text-align: center">
        ...
        <div class="tention"><span class="iconfont icon ">&#xe645;</span>请编辑信息</div>
        <div class="addrow">
        ...
        <div class="addrow">
          <el-input placeholder="" v-model="patieninfo.alladdress" >
            <template slot="prepend">所住省市区</template>
          </el-input>
        </div>
        <div class="addrow">
          <el-input placeholder="" v-model="patieninfo.adress" >
        ...
        <div style="height: 50px;line-height: 50px;background: white;border-bottom:1px solid #F2F2F2 ">
          ...
          <input type="radio" value="1"  v-model="patieninfo.defaultPatient" @click="isSendMessage" style="float: right;margin-right: 20px;margin-top: 18px">
        </div>
          <button type="submit" class="addpatien" @click="updatapatien">保存</button>
...
export default {
  ...
  data() {
    return{
       patieninfo:{
              patientName:this.$route.params.patien.patientName,
              idcardNo:this.$route.params.patien.idcardNo,
              phone:this.$route.params.patien.phone,
              gender:this.$route.params.patien.gender,
              relationship:this.$route.params.patien.relationship=='本人'?'本人':this.$route.params.patien.relationship=='爱人'?'爱人':this.$route.params.patien.relationship=='子女'?'子女':'其他',
              birthdate:this.$route.params.patien.birthdate,
              defaultPatient:this.$route.params.patien.defaultPatient,
              maritalStatus:this.$route.params.patien.maritalStatus,
              adress:this.$route.params.patien.adress,
              adressPro:this.$route.params.patien.adressPro,
              adressCountry:this.$route.params.patien.adressCountry,
              adressCity:this.$route.params.patien.adressCity,
              alladdress:`${this.$route.params.patien.adressPro},${this.$route.params.patien.adressCountry},${this.$route.params.patien.adressCity}`
       },
      num:0,
      marriy:[
        {
...
/*      console.log(this.num)*/
    ...
    updatapatien() {
      let tempaddres=this.patieninfo.alladdress.split(',')
      let this_=this
      axios.post( `/api/user/updatapatient?patientName=${this.patieninfo.patientName}&idcardNo=${this.patieninfo.idcardNo}&phone=${this.patieninfo.phone}&gender=${this.patieninfo.gender}&relationship=${this.patieninfo.relationship}&birthdate=${moment(this.patieninfo.birthdate).format('l')}&defaultPatient=${this.patieninfo.defaultPatient}&maritalStatus=${this.patieninfo.maritalStatus}&adressPro=${tempaddres[0]}&adressCountry=${tempaddres[1]}&adressCity=${tempaddres[2]}&adress=${this.patieninfo.adress}&userCode=${this.getLogUser.itemcode}&creater=${this.getLogUser.username}&itemcode=${this.$route.params.patien.itemcode}&itemid=${this.$route.params.patien.itemid}`)
